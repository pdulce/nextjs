...
const databaseFile = path.resolve(".", "resources/factUTEDBLite.db");
const monthList = [
  "Enero",
  "Febrero",
  "Marzo",
  ...
  return new Promise((resolve) => {
    db.all(myQuery, (err, rows) => {
      ...
      rows.forEach(function (row) {
        //${index}->
        arr.push(
          `${row.Proyecto} | ${row.mes} | ${row.Cod_GEDEON} | ${row.desc}`
        );
      });
      resolve(arr);
    }); //end of db.all
...
var queryCertifMensualAT = function (arr) {
  ...
  var myQuery =
    "SELECT i99.Proyecto_ID as Proyecto, i99.id as Cod_GEDEON, i99.Titulo as desc, date(i99.Fecha_de_tramitacion) as fechatram FROM incidenciasProyecto i99 LEFT OUTER JOIN subdireccion s74 ON s74.id=i99.Unidad_origen LEFT OUTER JOIN servicio s68 ON s68.id=i99.Area_origen  WHERE (" +
  ...
  return new Promise((resolve) => {
    db.all(myQuery, (err, rows) => {
      ...
      rows.forEach(function (row) {
        //${index}->
        arr.push(
          `${row.Proyecto} | ${row.Cod_GEDEON} | ${row.desc} | [${row.fechatram}]`
        );
      });
      resolve(arr);
    }); //end of db.all
...
var genCertifMensualAT = function (records, mesCertificado) {
  ...
  bloques.append(
    "Número total de actividades realizadas en " +
      monthList[parseInt(mesCertificado) - 1] +
      ": " +
      records.length
  );
  bloques.append("\n");
...
var genReportCaducadas = function (records, diasfin) {
  ...
  bloques.append(
    `Número total de actividades que van a finalizar en los próximos ${diasfin} días => ${records.length}`
  );
  bloques.append("\n");
      ...
      datasets: [
        {
          ...
          //[(123.5+104)+(57+33.75),(222.75+192.65),(99+23)+(56.25+54.15)], /**FOM2 */
          data: [26 + 3.25, 2] /**FRMA */,
...
module.exports = {
  ...
  queryReportCUBO,
  genReportCUBO,
  queryCertifMensualAT,
  genCertifMensualAT,
  queryReportPPTX,
  genReportPPTX,
  genJSONReportPPTX,
  queryReportCaducadas,
  genReportCaducadas,
};
